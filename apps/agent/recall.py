...
_FILTER_PROMPT = """\
사용자 질문에 답하기 위해 *반드시* 필요한 기억만 keep 하세요.
*기본 동작은 SKIP* 입니다 — 명확하게 직접 관련된 것만 keep.
...
엄격한 규칙:
1. 질문이 단순 행동 요청 ("X 추가", "X 삭제", "X 저장", "Y 라고 메모") 이면 — 그 X/Y 자체와 직접 관련된 항목만 keep. 다른 인물·주제는 모두 SKIP.
2. 질문이 정보 요청이면 — 그 답변을 *직접* 만드는 데 필요한 항목만 keep.
3. 인물·주제가 같다는 이유만으로 keep 하지 말 것. 의미가 직접 연결되어야 함.
4. 의심스러우면 SKIP. 보수적으로 좁히세요.
5. 빈 결과 (모두 SKIP) 도 완벽히 정상입니다.
